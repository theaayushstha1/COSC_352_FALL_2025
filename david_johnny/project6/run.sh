...
IMAGE_NAME="project5-go-homicide:latest"
...
echo "Running Go program inside Docker..."
...
if [ -n "$OUTPUT_FORMAT" ]; then
  ...
  docker run --rm -v "${WIN_DIRNAME}:/output" "$IMAGE_NAME" ./homicide-parser "$OUTPUT_FORMAT"
