...
object Main {
  ...
  val TargetYear = 2025
  val City = "Baltimore"          // used in standardized records
  val Offense = "Homicide"        // used in standardized records

  // -----------------------------
  // CLI flag: --output=csv|json (default: stdout)
  // -----------------------------
  private def parseFormat(args: Array[String]): Format = {
    val joined = args.mkString(" ")
    val eq     = """--output=(\w+)""".r.findFirstMatchIn(joined).map(_.group(1))
    val spaced = args.sliding(2).find(a => a.head == "--output").map(_(1))
    (eq orElse spaced).map(_.toLowerCase) match {
      case Some("csv")  => Csv
      case Some("json") => Json
      case _            => Stdout
    }
  ...
  def fetch(url: String, connectTimeoutMs: Int = 15000, readTimeoutMs: Int = 20000): String = {
    ...
    conn.setRequestProperty("User-Agent", "Mozilla/5.0 (Project5-Scala/2.0)")
  ...
  def main(args: Array[String]): Unit = {
    val format = parseFormat(args) // csv | json | stdout(default)
    ...
    val cumulativeRows = monthsPresent.map { m =>
      ...
      (m, f"$TargetYear%04d-$m%02d", count, avgToDate, diff, pctDiff)
    }

    // =========================
    // QUESTION 2 (18–24 by gender)
    // =========================
    ...
    // -----------------------
    //  A) DEFAULT STDOUT (unchanged style)
    // -----------------------
    if (format == Stdout) {
      ...
      cumulativeRows.foreach { case (m, label, count, avg, diff, pct) =>
        val avgStr = f"$avg%.2f"
    ...
    // -----------------------
    //  B) STANDARDIZED RECORDS (for csv/json)
    // -----------------------
    // Q1 rows: one per month
    val q1Records: Seq[Map[String, Any]] = cumulativeRows.map {
      case (mNum, label, count, avg, diff, pct) =>
        Map(
          "record_type"   -> "monthly_counts",
          "city"          -> City,
          "year"          -> TargetYear,
          "month"         -> label.takeRight(2),   // "01", "02", ...
          "offense"       -> Offense,
          "count"         -> count,
          "avg_to_date"   -> BigDecimal(avg).setScale(2, BigDecimal.RoundingMode.HALF_UP).toString,
          "diff_from_avg" -> BigDecimal(diff).setScale(2, BigDecimal.RoundingMode.HALF_UP).toString,
          "percent_diff"  -> pct                    // already like "12.3%"
        )
    }

    // Q2 rows: one per gender (+ one ratio row)
    val q2Records: Seq[Map[String, Any]] = Seq(
      Map(
        "record_type"    -> "age_18_24_share",
        "city"           -> City,
        "year"           -> TargetYear,
        "gender"         -> "Male",
        "total_known"    -> male.total,
        "age_18_24"      -> male.youngCount,
        "percent_18_24"  -> f"${male.pct}%.1f%%"
      ),
      Map(
        "record_type"    -> "age_18_24_share",
        "city"           -> City,
        "year"           -> TargetYear,
        "gender"         -> "Female",
        "total_known"    -> female.total,
        "age_18_24"      -> female.youngCount,
        "percent_18_24"  -> f"${female.pct}%.1f%%"
      ),
      Map(
        "record_type"          -> "age_18_24_share_ratio",
        "city"                 -> City,
        "year"                 -> TargetYear,
        "ratio_male_to_female" -> ratioTxt
      )
    )

    val results: Seq[Map[String, Any]] = q1Records ++ q2Records

    // Write CSV/JSON (files in ./output/)
    OutputFormatter.write(
      results,
      Config(format = format, outDir = "output", baseName = "results")
    )
  }
}
