...
// COSC 352
// Command-line tool that searches a PDF (e.g., Morgan 2030) for passages
// relevant to a user query. Uses PyPDF2 (Python) for text extraction and
// implements a TF–IDF-style scoring scheme with length normalization.
//
// Usage:
...
fn main() raises:
    ...
    # passages is a Python list of dicts: {'text': ..., 'page': ..., 'score': ...}
    ...
    # Tokenize query to get keywords (min length 3)
    ...
    # Compute TF-IDF-like scores with length normalization
    for passage in passages:
        var text_lower = passage["text"].lower()
        ...
        for term in query_terms:
            ...
            if doc_freq > 0:
                idf = math.log(
                    builtins.float(total_passages) / builtins.float(doc_freq)
                )

            score = score + (tf * idf)

        # Length normalization so long passages don't auto-win
        if passage_length > 0:
    ...
    for i in range(n):
        ...
        # If score is not positive, no more meaningful results
        if score <= 0.0:
            break
