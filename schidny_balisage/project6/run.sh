...
# Script to run Baltimore Crime Analysis (Go version) with optional output format
# Usage: 
...
if [ -z "$OUTPUT_ARG" ]; then
    ...
    # Output argument provided, pass it to Go program and copy output files back
    OUTPUT_FORMAT="${OUTPUT_ARG#*=}"
    
    # Run container and copy output file
    CONTAINER_ID=$(docker run -d baltimore-analysis-go $OUTPUT_ARG)
