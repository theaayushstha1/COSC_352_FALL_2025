"""
SIMD-optimized search core using NumPy vectorization
This simulates SIMD operations using NumPy arrays
"""
import sys
import math
import numpy as np
...
class SearchEngineSIMD:
    """SIMD-optimized search using NumPy vectorization"""
    def __init__(self):
        self.passages = []
    
    def tokenize_simd(self, text):
        """Vectorized tokenization using NumPy for SIMD-like performance"""
        # Convert to numpy array for vectorized operations
        text_array = np.array(list(text.lower()))
        
        # Vectorized character classification
        is_alpha = np.char.isalpha(text_array)
        is_digit = np.char.isdigit(text_array)
        is_alnum = is_alpha | is_digit
        
        # Build tokens from vectorized results
        tokens = []
        current_word = ""
    ...
    def split_into_passages(self, pages_text):
        ...
        for page_num, page_text in pages_text:
            tokens = self.tokenize_simd(page_text)
            start = 0
    ...
    def calculate_tf_simd(self, term, passage_tokens):
        """Vectorized TF calculation"""
        # Use NumPy for vectorized counting
        tokens_array = np.array(passage_tokens)
        count = np.sum(tokens_array == term)
    ...
    def calculate_idf(self, term):
        ...
        for passage in self.passages:
            tokens = self.tokenize_simd(passage.text)
            if term in tokens:
                doc_count += 1
        if doc_count == 0:
            return 0.0
    ...
    def calculate_passage_score(self, passage, query_terms):
        passage_tokens = self.tokenize_simd(passage.text)
        score = 0.0
        for term in query_terms:
            if self.is_stopword(term):
                continue
            tf = self.calculate_tf_simd(term, passage_tokens)
            if tf > 0.0:
    ...
    def search(self, query, top_n):
        query_terms = self.tokenize_simd(query)
        for passage in self.passages:
...
def run_search(pdf_path, query, num_results):
    ...
    engine = SearchEngineSIMD()
    print("Creating passages (SIMD optimized)...")
    engine.split_into_passages(pages_text)
    print(f"✓ Created {len(engine.passages)} passages\n")
    
    print("Searching (SIMD optimized)...")
