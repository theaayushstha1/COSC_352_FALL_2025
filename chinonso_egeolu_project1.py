...
1. Download the Wikipedia page:
   - Using wget: `wget https://en.wikipedia.org/wiki/Comparison_of_programming_languages -O input.html`
   - Or save the page source from your browser as "input.html".
2. Save this script as `chinonso_egeolu_project1.py`.
3. Run: `python hinonso_egeolu_project1.py input.html`
4. Output files (table_*.csv) will be generated in the current directory.
5. Load any CSV into a spreadsheet (e.g., Excel, Google Sheets) to view.
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python table_to_csv_stack.py <html_file>")
        print("Example: python table_to_csv_stack.py input.html")
        sys.exit(1)
