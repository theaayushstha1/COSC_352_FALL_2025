...
def main():
    if len(sys.argv) != 3:
        print("Usage: python read_html_table.py <URL_or_file> <output_prefix>")
        return
    src = sys.argv[1]
    prefix = sys.argv[2]
    try:
    ...
    if not parser.tables:
        ...
        return
    files = write_csv(parser.tables, prefix)
    for fn in files:
        print(f"[INFO] Saved: {os.path.basename(fn)}")
