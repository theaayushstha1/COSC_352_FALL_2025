...
object BaltimoreHomicideAnalysis {
  ...
  // Directory for output files - fixed to work with Docker volume mount
  def getOutputDir(): String = {
    // Check if output subdirectory exists (for Docker volume mount)
    val outputSubdir = new File("/app/output")
    val dir = if (outputSubdir.exists() || outputSubdir.mkdirs()) {
      outputSubdir
    } else {
      // Fall back to /app if output subdirectory can't be created
      new File("/app")
    }
  ...
  def analyzeData(homicides: List[Homicide], format: String): Unit = {
    ...
    format.toLowerCase match {
      ...
      case _ =>
        ...
        println(s"\n=== Stabbing Victims 2025 (${finalStabbingRecords.length} records) ===")
        finalStabbingRecords.foreach(println)
        println(s"\n=== East Baltimore Victims (${finalEastBaltimoreRecords.length} records) ===")
        finalEastBaltimoreRecords.foreach(println)
    }
  }
